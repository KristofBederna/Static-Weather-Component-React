  ...
  return (
    <ul className="gap-5 flex flex-row mt-20" id="citiesList">
      {weatherList.map(weather => (
        <li onClick={() => handleCityChange(weather.id)} key={weather.id} className="bg-blue-500 text-white p-2 text-lg cursor-pointer select-none rounded-lg hover:scale-105 transition-transform shadow-lg">
          {weather.name}
        </li>
      ))}
    </ul>
